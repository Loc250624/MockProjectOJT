...
public class StudentAssessmentServiceImpl implements StudentAssessmentService {
    ...
    private List<Integer> readQuestionIds(Map<String, Object> payload) {
        Object questionIdsValue = payload.get(QUESTION_IDS_FIELD);
        if (!(questionIdsValue instanceof Iterable<?> rawQuestionIds)) {
            return List.of();
        }
        List<Integer> questionIds = new ArrayList<>();
        for (Object rawQuestionId : rawQuestionIds) {
            Integer questionId = parseInteger(rawQuestionId);
            if (questionId != null && !questionIds.contains(questionId)) {
                questionIds.add(questionId);
                if (questionIds.size() == QUIZ_QUESTION_LIMIT) {
                    break;
                }
            }
        }
