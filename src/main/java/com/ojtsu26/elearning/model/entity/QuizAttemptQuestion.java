...
@Table(
        ...
        uniqueConstraints = {
                ...
                @UniqueConstraint(name = "uq_attempt_display_order", columnNames = {"attempt_id", "display_order"})
        },
        indexes = {
                @Index(name = "idx_attempt_question_order", columnList = "attempt_id,display_order"),
                @Index(name = "idx_attempt_question_question", columnList = "question_id")
        }
)
@Getter
@Setter
...
public class QuizAttemptQuestion {
    ...
    @Column(name = "points_snapshot", nullable = false, precision = 10, scale = 2)
